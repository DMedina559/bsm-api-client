...
from bsm_api_client.models import AddPlayersPayload, SettingItem, PruneDownloadsPayload, InstallServerPayload
...
async def test_get_info(client):
    ...
    with patch.object(client, "_request", new_callable=AsyncMock) as mock_request:
        ...
        mock_request.assert_called_once_with(method="GET", path="/info", authenticated=False)
...
async def test_scan_players(client):
    ...
    with patch.object(client, "_request", new_callable=AsyncMock) as mock_request:
        ...
        mock_request.assert_called_once_with(method="POST", path="/players/scan", authenticated=True)
...
async def test_get_players(client):
    ...
    with patch.object(client, "_request", new_callable=AsyncMock) as mock_request:
        mock_request.return_value = {"status": "success", "players": [{"name": "player1", "xuid": "123"}]}
        result = await client.async_get_players()
        mock_request.assert_called_once_with(method="GET", path="/players/get", authenticated=True)
...
async def test_get_all_settings(client):
    ...
    with patch.object(client, "_request", new_callable=AsyncMock) as mock_request:
        mock_request.return_value = {"status": "success", "settings": {"web": {"port": 8080}}}
        result = await client.async_get_all_settings()
        mock_request.assert_called_once_with(method="GET", path="/settings", authenticated=True)
...
async def test_reload_settings(client):
    ...
    with patch.object(client, "_request", new_callable=AsyncMock) as mock_request:
        mock_request.return_value = {"status": "success", "message": "Settings reloaded."}
        result = await client.async_reload_settings()
        mock_request.assert_called_once_with(method="POST", path="/settings/reload", authenticated=True)
        assert result["status"] == "success"
...
async def test_prune_downloads(client):
    ...
    with patch.object(client, "_request", new_callable=AsyncMock) as mock_request:
        ...
        mock_request.return_value = {"status": "success", "message": "Downloads pruned."}
...
async def test_install_new_server(client):
    ...
    with patch.object(client, "_request", new_callable=AsyncMock) as mock_request:
        payload = InstallServerPayload(server_name="test-server", server_version="LATEST", overwrite=True)
        mock_request.return_value = {"status": "success", "message": "Server installed."}
