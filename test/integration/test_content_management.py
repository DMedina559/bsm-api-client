...
async def content_files():
    ...
    yield {
        "world_file": dummy_world_file,
        "addon_file": dummy_addon_file,
    }
...
async def wait_for_server_status(client, server_name, is_running, timeout=60):
    ...
    pytest.fail(f"Server did not enter '{status_str}' state within {timeout} seconds.")
...
class TestContentManagement:
    ...
    async def test_backup_and_restore(self, server, bedrock_server):
        """
        Tests creating a backup, listing it, and then restoring it.
        This test requires starting the server.
        """
        ...
        try:
            ...
            await wait_for_server_status(client, server_name, is_running=True)
            ...
            list_response = None
            for _ in range(60):
                list_response = await client.async_list_server_backups(
                    server_name, "world"
                )
    ...
    async def test_world_and_addon_management(
        self, server, bedrock_server, content_files
    ):
        """
        Tests listing and installing worlds and addons. This does not require a running server.
        """
        client = BedrockServerManagerApi(server, "admin", "password")
        server_name = bedrock_server
    ...
    async def test_other_content_actions(self, server, bedrock_server):
        """
        ...
        try:
            ...
            assert prune_result.status in [
                "success",
                "pending",
            ]  # Corrected this assertion
