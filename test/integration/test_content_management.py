...
class TestContentManagement:
    ...
    async def test_backup_and_restore(self, bedrock_server, wait_for_server_status, content_client):
        """
        ...
        backup_result = await client.async_trigger_server_backup(server_name, backup_payload)
        ...
        restore_payload = RestoreActionPayload(restore_type="world", backup_file=backup_file)
        restore_result = await client.async_restore_server_backup(server_name, restore_payload)
        assert restore_result.status in ["success", "pending"]

    async def test_world_and_addon_management(self, bedrock_server, content_files, content_client):
        """
        ...
        install_world_result = await client.async_install_server_world(server_name, install_world_payload)
        ...
        install_addon_result = await client.async_install_server_addon(server_name, install_addon_payload)
        assert install_addon_result.status in ["success", "pending"]

    async def test_other_content_actions(self, bedrock_server, wait_for_server_status, content_client):
        """
