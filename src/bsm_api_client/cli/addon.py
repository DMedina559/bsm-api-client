...
from bsm_api_client.models import FileNamePayload, AddonActionPayload, AddonSubpackPayload, AddonReorderPayload
...
@pass_async_context
async def manage_addons(ctx, server_name: str):
    ...
    while True:
        try:
            ...
            if not addons:
                click.secho("Failed to fetch installed addons or no addons returned.", fg="red")
                return
            ...
            click.clear()
            click.secho(f"--- Manage Addons for {server_name} ---", fg="magenta", bold=True)
            ...
            for idx, pack in enumerate(bp, 1):
                ...
                menu_choices.append(f"{idx}. {status} [BP] {pack.name} (v{'.'.join(map(str, pack.version))})")
                
            menu_choices.append(Separator("--- Resource Packs ---"))
            for idx, pack in enumerate(rp, 1):
                status = "🟢" if pack.status == "ACTIVE" else "⚪"
                menu_choices.append(f"{idx}. {status} [RP] {pack.name} (v{'.'.join(map(str, pack.version))})")
                
            menu_choices.extend([
                Separator("--- Actions ---"),
                "Reorder Behavior Packs",
                "Reorder Resource Packs",
                "Back"
            ])
            
            choice = await questionary.select(
                "Select an addon to manage or an action:",
            ...
            if choice == "Reorder Behavior Packs":
                ...
                if len(ordered_uuids) == len(active_bp):
                    payload = AddonReorderPayload(pack_type="behavior", uuids=ordered_uuids)
                    res = await client.async_reorder_server_addon(server_name, payload)
                    if res.task_id:
                        await monitor_task(client, res.task_id, "Reorder successful", "Failed to reorder")
                    else:
            ...
            if choice == "Reorder Resource Packs":
                ...
                if len(ordered_uuids) == len(active_rp):
                    payload = AddonReorderPayload(pack_type="resource", uuids=ordered_uuids)
                    res = await client.async_reorder_server_addon(server_name, payload)
                    if res.task_id:
                        await monitor_task(client, res.task_id, "Reorder successful", "Failed to reorder")
                    else:
            ...
            if action_choice == "Enable":
                res = await client.async_enable_server_addon(server_name, AddonActionPayload(pack_uuid=pack.uuid, pack_type=pack_type))
                if res.task_id:
                    await monitor_task(client, res.task_id, "Enabled successfully", "Failed to enable")
            elif action_choice == "Disable":
                res = await client.async_disable_server_addon(server_name, AddonActionPayload(pack_uuid=pack.uuid, pack_type=pack_type))
                if res.task_id:
                    await monitor_task(client, res.task_id, "Disabled successfully", "Failed to disable")
            elif action_choice == "Uninstall":
                if await questionary.confirm(f"Are you sure you want to uninstall {pack.name}?").ask_async():
                    res = await client.async_uninstall_server_addon(server_name, AddonActionPayload(pack_uuid=pack.uuid, pack_type=pack_type))
                    if res.task_id:
                        await monitor_task(client, res.task_id, "Uninstalled successfully", "Failed to uninstall")
                ...
                sp_choice = await questionary.select(
                    "Select new subpack:", 
                    choices=subpack_choices, 
                    default=default_sp
                ).ask_async()
                
                if sp_choice:
                    clean_sp_choice = sp_choice.replace(" (Active)", "")
                    selected_sp = next((sp for sp in pack.subpacks if sp.get("name") == clean_sp_choice or sp.get("folder_name") == clean_sp_choice), None)
                    if selected_sp:
                        ...
                        setattr(payload, f"subpack_{pack.uuid}", selected_sp.get("folder_name"))

                        res = await client.async_update_server_addon_subpack(server_name, payload)
                        if res.task_id:
                            await monitor_task(client, res.task_id, "Subpack updated successfully", "Failed to update subpack")
